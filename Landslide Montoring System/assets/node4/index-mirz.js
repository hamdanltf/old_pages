...
async function main() {
    ...
    db.ref('/SENSOR2').endAt().limitToLast(1).on('child_added', function (snapshot) {
        if (FETCH_STATUS) {
            ...
            chart.data.datasets[0].data.push(parameter.miring_z)
    ...
    var ctx = document.getElementById('myChart-z').getContext('2d');
    var chart = new Chart(ctx, {
        ...
        data: {
            ...
            datasets: [{
                label: "Kemiringan Sumbu Z",
                fill: false,
                borderColor: 'rgb(255, 255, 0)',
                data: FILTERED_DATA.map(data => data.parameter.miring_z),
            }]
        },
